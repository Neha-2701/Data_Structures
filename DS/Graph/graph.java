...
public class graph{
    ...
    public static int serachVtx(int u,int v)
    {
    ...
    {
        int l1=serachVtx(u, v);
        graph[u].remove(l1);

        int l2=serachVtx(v,u);
        graph[v].remove(l2);
    }
    ...
    public static int allPath(int src,int dest,boolean[] vis,String psf,String wsf)
    {
        if(src==dest)
        {
    ...
    {
        if(src==dest) return new pair(src+" ",0,true);
        vis[src]=true;
